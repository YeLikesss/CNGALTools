...
using StaticExtractor.Instruction;

namespace StaticExtractor
{
    internal class SwecCrypto
    {
        private readonly byte[] mTable_1 = new byte[]
        {
            ...
            0x22, 0x35, 0x2E, 0x2A, 0x32, 0x24, 0x1D, 0x20, 0x07, 0x0D, 0x0E, 0x03, 0x00, 0x06, 0x09, 0x0A,
            ...
            0x09, 0x04, 0x05, 0x0B, 0x0C, 0x07, 0x02, 0x0E, 0x0A, 0x00, 0x09, 0x0E, 0x06, 0x03, 0x0F, 0x05,
            ...
            0x04, 0x0F, 0x0E, 0x03, 0x0B, 0x05, 0x02, 0x0C, 0x0F, 0x01, 0x08, 0x0E, 0x06, 0x0B, 0x03, 0x04,
            ...
            0x0B, 0x06, 0x07, 0x0C, 0x00, 0x05, 0x0E, 0x09, 0x0E, 0x04, 0x0D, 0x01, 0x02, 0x0F, 0x0B, 0x08,
            ...
            0x05, 0x0B, 0x03, 0x0E, 0x0A, 0x00, 0x06, 0x0D, 0x0D, 0x02, 0x08, 0x04, 0x06, 0x0F, 0x0B, 0x01,
            ...
            0x0F, 0x0C, 0x09, 0x00, 0x03, 0x05, 0x06, 0x0B, 0x04, 0x0B, 0x02, 0x0E, 0x0F, 0x00, 0x08, 0x0D,
            ...
            0x09, 0x05, 0x00, 0x0F, 0x0E, 0x02, 0x03, 0x0C, 0x0C, 0x01, 0x0A, 0x0F, 0x09, 0x02, 0x06, 0x08,
            ...
            0x0B, 0x0E, 0x01, 0x07, 0x06, 0x00, 0x08, 0x0D, 0x02, 0x0C, 0x04, 0x01, 0x07, 0x0A, 0x0B, 0x06,
            ...
            0x06, 0x0F, 0x00, 0x09, 0x0A, 0x04, 0x05, 0x03, 0x10, 0x07, 0x14, 0x15, 0x1D, 0x0C, 0x1C, 0x11,
            0x01, 0x0F, 0x17, 0x1A, 0x05, 0x12, 0x1F, 0x0A, 0x02, 0x08, 0x18, 0x0E, 0x20, 0x1B, 0x03, 0x09,
            0x13, 0x0D, 0x1E, 0x06, 0x16, 0x0B, 0x04, 0x19
        };
        private readonly byte[] mTable_3 = new byte[]
        {
        ...
        private readonly byte[] mBlock = new byte[0x1120];

        public SwecCrypto()
        {
            string key = "SwecKey";
            string lic = "SwecLicenseSystem";

            string sk = this.GenerateKey(Encoding.ASCII.GetBytes(lic), Encoding.ASCII.GetBytes(key));
            this.mSalt = SHA1.HashData(Encoding.ASCII.GetBytes(sk));
        }
        ...
        public void Decrypt(Span<byte> data)
        {
            ...
            {
                string skey = "SwecKey";
                string slic = data.Length.ToString();
                string gkey = this.GenerateKey(Encoding.ASCII.GetBytes(slic), Encoding.ASCII.GetBytes(skey));

                byte[] k1 = this.mSalt;
                byte[] k2 = Encoding.ASCII.GetBytes(gkey);
                
                byte[] k3 = new byte[k1.Length + k2.Length];
                Array.Copy(k1, 0, k3, 0, k1.Length);
                Array.Copy(k2, 0, k3, k1.Length, k2.Length);

                key = SHA1.HashData(k3);
            }
            ...
            {
                //解密前后512KB
                this.Decrypt(data[..0x80000], key);
                this.Decrypt(data[^0x80000..], key);

                //文件头解密12字节
                SwecCrypto.DecryptSample(MemoryMarshal.Cast<byte, uint>(data[..0xC]), (uint)data.Length);
            }
        }
        ...
        private void Decrypt(Span<byte> data, byte[] key)
        {
            Span<byte> block = this.mBlock;

            //0x0 - 0x1090
            this.TransBlock(key, 0x10);
            //0x1090 - 0x10A0
            {
                ReadOnlySpan<byte> lb = block[..0x1000];
                ReadOnlySpan<byte> sb = block.Slice(0x1048, 0x48);
                Span<byte> blk = block.Slice(0x1090, 0x10);
                blk.Clear();

                ulong k = 0UL;
                k = SwecCrypto.Transform3(k, lb, sb);

                MemoryMarshal.Write(blk[..8], ref k);
                MemoryMarshal.Write(blk[8..16], ref k);
            }

            Span<byte> decBlock = block.Slice(0x1090, 0x10);
            for(int i = 0; i < data.Length; ++i)
            {
                ulong val = SwecCrypto.Transform3(MemoryMarshal.Read<ulong>(decBlock), block[..0x1000], block.Slice(0x1048, 0x48));
                MemoryMarshal.Write(decBlock, ref val);

                byte b = data[i];
                data[i] = (byte)(val ^ b);

                decBlock[9..16].CopyTo(decBlock[0..7]);
                decBlock[7] = b;
                decBlock[0..8].CopyTo(decBlock[8..16]);
            }
        }
        ...
        /// <param name="key">key</param>
        private unsafe string GenerateKey(byte[] lic, byte[] key)
        {
            ReadOnlySpan<byte> ctxTable = this.mTable_1;

            Span<ulong> tempTbl = stackalloc ulong[0x10];
            {
                ulong skey;
                {
                    Span<byte> k = stackalloc byte[8];
                    key.AsSpan()[..Math.Min(key.Length, 8)].CopyTo(k);
                    //0xB0 - 0xE8
                    skey = SwecCrypto.Transform1(ctxTable.Slice(0xB0, 0x38),
                                                BinaryPrimitives.ReverseEndianness(BitConverter.ToUInt32(k)),
                                                BinaryPrimitives.ReverseEndianness(BitConverter.ToUInt32(k[4..])),
                                                0x20, 0x1C);
                }
                {
                    Span<uint> sk32 = stackalloc uint[2];
                    MemoryMarshal.Write(MemoryMarshal.AsBytes(sk32), ref skey);

                    //0xE8 - 0xF8
                    ReadOnlySpan<byte> k = ctxTable.Slice(0xE8, 0x10);
                    for(int i = 0; i < 0x10; ++i)
                    {
                        ...
                        for(int j = 0; j < 2; ++j)
                        {
                            ...
                            v1 = sk32[j];
                            v1 = BitOperations.RotateLeft(v1, val + 4);
                            v2 = v1 >> 4;
                            v1 = val == 1 ? v1 & 1u : v1 & 3u;

                            sk32[j] = v1 + v2;
                        }

                        //0xF8 - 0x128
                        tempTbl[i] = SwecCrypto.Transform1(ctxTable.Slice(0xF8, 0x30), sk32[1], sk32[0], 0x1C, 0x18);
                    }
                }
            }
...

            {
                ...
                {
                    ulong inData = 0UL;
                    Span<byte> block = new(&inData, sizeof(ulong));
                    for(int j = 0; j < block.Length; ++j)
                    {
                        if (keyPos == lic.Length)
                        {
                            break;
                        }
                        byte k = lic[keyPos++];
                    ...
                    //0x00 - 0x40
                    ulong k1 = SwecCrypto.Transform1(ctxTable[..0x40],
                                                    BinaryPrimitives.ReverseEndianness((uint)inData),
                                                    BinaryPrimitives.ReverseEndianness((uint)(inData >> 32)),
                                                    0x20, 0x20);
                    Span<uint> k1_block = new(&k1, sizeof(ulong));
                    for(int j = 0; j < 0x10; ++j)
                    {
                        ulong k2;

                        //0x40 - 0x70
                        k2 = SwecCrypto.Transform1(ctxTable.Slice(0x40, 0x30), (ushort)k1_block[0], (ushort)(k1_block[0] >> 16), 0x10, 0x18);
                        k2 ^= BitOperations.RotateLeft(tempTbl[j], 32);

                        Span<uint> k2_block = new(&k2, sizeof(ulong));

                        //0x128 - 0x328
                        ReadOnlySpan<byte> tempTbl_2 = ctxTable.Slice(0x128, 0x200);
                        for(int a = 0; a < 2; ++a)
                        {
                            uint val = k2_block[a];
                            uint res = 0u;
                            for(int b = 0; b < 4; ++b)
                            {
                                uint v1 = SwecCrypto.Transform2(tempTbl_2.Slice((a * 4 + b) * 0x40, 0x40), (byte)(val & 0x3F));
                                ulong v2 = ((ulong)v1 << 32) | res;
                                v2 >>= 4;
                                res = (uint)v2;
                        ...
                        //0x328 - 0x348
                        k2 = SwecCrypto.Transform1(ctxTable.Slice(0x328, 0x20), k2_block[0] >> 16, k2_block[1] >> 16, 0x10, 0x10);

                        uint k3 = (uint)(((k2 >> 16) & 0xFFFF0000u) + k2);
                        k3 ^= k1_block[1];
                        k1_block[1] = k1_block[0];
                        k1_block[0] = k3;
                    }

                    //0x70 - 0xB0
                    ulong r1 = SwecCrypto.Transform1(ctxTable.Slice(0x70, 0x40), k1_block[0], k1_block[1], 0x20, 0x20);
                    ulong r2 = BinaryPrimitives.ReverseEndianness(r1);
                    MemoryMarshal.Write(result.AsSpan().Slice(i, 8), ref r2);
                }
            }
        ...
        private void TransBlock(byte[] key, int length)
        {
            byte[] hash = SHA1.HashData(key);

            Span<byte> block = this.mBlock;
            block.Clear();
            this.mTable_3.AsSpan().CopyTo(block);

            {
                ReadOnlySpan<uint> hash_block = MemoryMarshal.Cast<byte, uint>(hash);
                ReadOnlySpan<uint> src = MemoryMarshal.Cast<byte, uint>(block.Slice(0x1000, 0x48));
                Span<uint> dst = MemoryMarshal.Cast<byte, uint>(block.Slice(0x1048, 0x48));

                int hashPos = 0;
                for(int i = 0; i < 0x12; ++i)
                {
                    if(hashPos == hash_block.Length)
                    {
                        hashPos = 0;
                    }
                    dst[i] = src[i] ^ BinaryPrimitives.ReverseEndianness(hash_block[hashPos++]);
                }
            }

            ulong k1 = 0UL;
            {
                ReadOnlySpan<uint> src = MemoryMarshal.Cast<byte, uint>(block[..0x1000]);
                Span<uint> dst = MemoryMarshal.Cast<byte, uint>(block.Slice(0x1048, 0x48));
                for (int i = 0; i < 0x9; ++i)
                {
                    k1 = SwecCrypto.Transform3(k1, MemoryMarshal.AsBytes(src), MemoryMarshal.AsBytes(dst));

                    dst[i * 2 + 0] = BinaryPrimitives.ReverseEndianness((uint)k1);
                    dst[i * 2 + 1] = BinaryPrimitives.ReverseEndianness((uint)(k1 >> 32));
                }
            }
            {
                Span<uint> lb = MemoryMarshal.Cast<byte, uint>(block[..0x1000]);
                ReadOnlySpan<uint> sb = MemoryMarshal.Cast<byte, uint>(block.Slice(0x1048, 0x48));

                for (int i = 0; i < 0x200; ++i)
                {
                    k1 = SwecCrypto.Transform3(k1, MemoryMarshal.AsBytes(lb), MemoryMarshal.AsBytes(sb));

                    lb[i * 2 + 0] = BinaryPrimitives.ReverseEndianness((uint)k1);
                    lb[i * 2 + 1] = BinaryPrimitives.ReverseEndianness((uint)(k1 >> 32));
                }
            }
        }
        ...
        /// 变换1
        /// </summary>
        /// <param name="tbl">字节表 长度是 counter * 2</param>
        /// <param name="selector_1">选择子_1</param>
        /// <param name="selector_2">选择子_2</param>
        /// <param name="limiter">限制阈值</param>
        /// <param name="counter">计数器</param>
        public static ulong Transform1(ReadOnlySpan<byte> tbl, uint selector_1, uint selector_2, int limiter, int counter)
        {
            Span<uint> res = stackalloc uint[2];
            ...
            for (int i = 0; i < 2; ++i)
            {
                ...
                for(int idx = 0; idx < counter; ++idx)
                {
                    ...
                    {
                        val = limiter - val;
                        flag = BitTest.Bt(selector_1, (uint)val);
                    }
                    else
                    {
                        val = 2 * limiter - val;
                        flag = BitTest.Bt(selector_2, (uint)val);
                    }

                    if (flag)
                    {
                        res[i] = BitTest.Bts(res[i], (uint)(counter - idx - 1));
                    }
                }
            }
        ...
        /// 变换2
        /// </summary>
        /// <param name="tbl">字节表 长度0x40</param>
        /// <param name="selector">选择子</param>
        public static uint Transform2(ReadOnlySpan<byte> tbl, byte selector)
        {
            byte v1 = (byte)((selector & 0x1E) >> 1);
            byte v2;
            if ((selector & 0x20) != 0)
            {
                v2 = (byte)((selector | 2) & 3);
            }
            else
            {
                v2 = (byte)(selector & 1);
        ...
        /// <param name="tbl_1">表1 0x1000</param>
        /// <param name="tbl_2">表2 0x48</param>
        public unsafe static ulong Transform3(ulong val, ReadOnlySpan<byte> tbl_1, ReadOnlySpan<byte> tbl_2)
        {
            uint sk_lo = BinaryPrimitives.ReverseEndianness((uint)val);
            uint sk_hi = BinaryPrimitives.ReverseEndianness((uint)(val >> 32));

            ReadOnlySpan<uint> tbl_1_p4 = MemoryMarshal.Cast<byte, uint>(tbl_1);
            ReadOnlySpan<uint> tbl_2_p4 = MemoryMarshal.Cast<byte, uint>(tbl_2);
            for (int i = 0; i < 0x10; ++i)
            {
                sk_lo ^= tbl_2_p4[i];

                ReadOnlySpan<byte> selector = new(&sk_lo, sizeof(uint));

                uint v;
                v = tbl_1_p4[selector[3]];
                v += tbl_1_p4[selector[2] + 0x100];
                v ^= tbl_1_p4[selector[1] + 0x200];
                v += tbl_1_p4[selector[0] + 0x300];
                v ^= sk_hi;
            ...
            sk_lo = BinaryPrimitives.ReverseEndianness(sk_lo);
            sk_hi = BinaryPrimitives.ReverseEndianness(sk_hi);

            return ((ulong)sk_lo << 32) | sk_hi;
        }
        ...
        /// <param name="data">数据</param>
        /// <param name="key">key</param>
        private static void DecryptSample(Span<uint> data, uint key)
        {
            for(int i = 0; i < data.Length; ++i)
            {
                uint sik = (uint)(data.Length - i);
